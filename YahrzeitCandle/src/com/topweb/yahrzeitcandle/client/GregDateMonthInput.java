...
public class GregDateMonthInput {
	...
	GregDateMonthInput (int gregmonth) {
		...
		for (int i=0;i<YahrzeitCandle.greg_months.length;i++){
	...
	GregDateMonthInput(){
		...
		//Date today = new Date();
		int gregmonth=YahrzeitCandle.m_hebDate.getGregorianMonth(); //Integer.parseInt(DateTimeFormat.getFormat("M").format(today));
		m_month.setItemSelected(gregmonth-1, true);
	...
	m_month.addChangeHandler (YahrzeitCandle.m_changedate);
		...
		public void setMonth(int gregorianMonth) {
			// TODO Auto-generated method stub
			m_month.setItemSelected(gregorianMonth-1,true);
		}

		public int getMonth() {
			// TODO Auto-generated method stub
			return m_month.getSelectedIndex()+1;
		}
